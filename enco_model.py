...
class AdjacencyMatrix(object):
    def __init__(self, num_variables, device):
        ...
        self.gamma = nn.Parameter(torch.zeros(num_variables, num_variables, device=device, requires_grad=True)) 
        self.gamma.data[torch.arange(num_variables), torch.arange(num_variables)] = -9e15
        self.theta = nn.Parameter(torch.zeros(num_variables, num_variables, device=device, requires_grad=True))
        self.num_variables = num_variables
    
    @torch.no_grad()
    def edge_probs(self, temperature=1):
        return torch.sigmoid(temperature * self.gamma) * torch.sigmoid(temperature * self.theta)
    
    @torch.no_grad()
    def uncertainty(self, temperature=1):
        return self.edge_probs(temperature) * (1 - self.edge_probs(temperature))
