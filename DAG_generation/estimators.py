...
import sys
sys.path.append("../")
from DAG_generation.distributions import PlackettLuce
from DAG_generation.utils import logcumsumexp, reverse_logcumsumexp, smart_perm, make_permutation_matrix
from itertools import permutations


def to_z(logits, u=None, req_grads=True):
...
def relax(fb, b, logits, z, c, v=None, **kwargs):
    ...
    logits_expand = logits[None].expand(z.shape[0], -1)
    z = _reattach_z_to_new_logits(logits_expand, z)
    z_tilde = _to_z_tilde(logits_expand, b, v)
    c_z = c(z, **kwargs)
